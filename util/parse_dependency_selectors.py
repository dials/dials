...
def preprocess_for_bootstrap(
    paths, prebuilt_cctbx, platform=None, sections=None, add_channel=True
):
    # type: (list[str | os.PathLike], bool, str | None, list[SectionName]|None, bool) -> list[str]
    """
    ...
    for pkg, ver, _ in sorted(merged_req, key=lambda x: x[0]):
        if pkg == "python":
            ...
            continue
        prefix = "conda-forge::" if add_channel else ""
        output_lines.append(prefix + pkg + (ver or ""))
    return output_lines
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-channel",
        help="Do not prefix channel name on output dependencies",
        action="store_false",
        dest="add_channel",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--header", help="Write a 'This is auto generated' header", action="store_true"
    )
...
#
# Please do not make manual changes to this file as it will be
# overwritten, instead please edit dependencies.yaml directly.
#
# This file generated by the command:
#
#   parse_dependency_selectors.py {}
#
#########################################################################""".format(
                " ".join(sys.argv)
            )
