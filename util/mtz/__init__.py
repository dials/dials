...
if tuple(ext.cmtz_struct_sizes()) not in expected_cmtz_struct_sizes:
    warnings.warn(
        """Unexpected iotbx.mtz.cmtz_struct_sizes(): %s
...
assert sorted(column_type_as_miller_array_type_hints.keys()) == sorted(
    column_type_legend.keys()
)
...
def default_column_types(miller_array):
    ...
    if miller_array.is_complex_array() and miller_array.sigmas() is None:
        ...
        miller_array.is_bool_array() or miller_array.is_integer_array()
        ...
        miller_array.is_real_array() and miller_array.sigmas() is not None
    ):
        if miller_array.anomalous_flag():
            result = "G"
            if miller_array.sigmas() is not None:
...
def are_anomalous_labels(sign, labels):
    for label in labels:
        if not is_anomalous_label(sign, label):
            return False
    return True
...
class label_decorator(__builtins__["object"]):
    ...
    def sigmas(self, root_label, anomalous_sign=None):
        return self.anomalous(
            self.sigmas_prefix + root_label + self.sigmas_suffix, anomalous_sign
        )

    def delta_anomalous(self, root_label):
        label = self.delta_anomalous_prefix + root_label + self.delta_anomalous_suffix
        self.check_character_length(label)
        return label
    ...
    def delta_anomalous_isym(self, root_label):
        label = (
            self.delta_anomalous_isym_prefix
            + root_label
    ...
    def phases(self, root_label, anomalous_sign=None):
        return self.anomalous(
            self.phases_prefix + root_label + self.phases_suffix, anomalous_sign
        )

    def hendrickson_lattman(self, root_label, i_coeff, anomalous_sign=None):
        assert 0 <= i_coeff < 4
        return self.anomalous(
            root_label + self.hendrickson_lattman_suffix_list[i_coeff], anomalous_sign
        )
...
class ccp4_label_decorator(label_decorator):
    def phases(self, root_label, anomalous_sign=None):
        ...
        return self.anomalous("PH" + root_label + self.phases_suffix, anomalous_sign)
...
def tidy_show_column_data_format_keyword(input):
    if input is None:
        return show_column_data_format_keywords[0]
    input = input.lower()
    for k in show_column_data_format_keywords:
        if input.startswith(k[0]):
            return k
    raise Sorry(
...
class _:
    ...
    def set_space_group_info(self, space_group_info, symbol=None):
        ...
        self.set_lattice_centring_type(symbol=group.conventional_centring_type_symbol())
    ...
    def set_hkl_base(self, unit_cell):
        ...
        return (
            self.add_crystal(
                name="HKL_base", project_name="HKL_base", unit_cell=unit_cell
            )
            .set_id(id=0)
            .add_dataset(name="HKL_base", wavelength=0)
            .set_id(id=0)
        )
    ...
    def show_summary(self, out=None, prefix=""):
        ...
        if self.n_crystals() > 0 and self.n_reflections() > 0:
            print(
                p + "Resolution range: %.6g %.6g" % self.max_min_resolution(), file=out
            )
    ...
    def show_column_data_human_readable(self, out=None):
        ...
        if miller_indices.size() == 0:
            ...
            h_width = len(
                "-%d"
                % int(
                    flex.max(flex.abs(miller_indices.as_vec3_double().as_double()))
                    + 0.5
                )
            )
        ...
        def show_labels():
            for i in range(0, len(labels), 4):
                print(
                    " ".join([h_blank] + ["%15s" % x for x in labels[i : i + 4]]),
                    file=out,
                )
            print(file=out)
    ...
    def show_column_data_spreadsheet(self, out=None):
        ...
        miller_indices, labels, pairs = self._show_column_data_preparation()
        print(
            ",".join([label.replace(",", "_") for label in ["H", "K", "L"] + labels]),
            file=out,
        )
        for iref, h in enumerate(miller_indices):
    ...
    def change_basis_in_place(
        self, cb_op, new_space_group_info=None, assert_is_compatible_unit_cell=False
    ):
        ...
        for crystal in self.crystals():
            ...
            crystal.set_unit_cell_parameters(crystal_symmetry.unit_cell().parameters())
            ...
            for dataset in crystal.datasets():
                ...
                for column_group in column_groups:
                    if (
                        ...
                        if (
                            merged_column_group.indices().size()
    ...
    """
        miller_arrays = self.as_miller_arrays(
            crystal_symmetry, force_symmetry, merge_equivalents, base_array_info
        )
        keys = []
        for crystal in self.crystals():
            for dataset in crystal.datasets():
    ...
    def group_columns(
        ...
        while 1:
            i_column += 1
            if i_column == len(all_columns):
                break
            if all_column_prev_use_flags[i_column]:
                continue
            ...
            if t0 in "BYI":  # integer columns
                ...
                labels = [l0]
                if (
                    i_column + 1 < len(all_column_types)
...
def column_group(
    ...
    if sigmas is not None:
        assert sigmas.size() == group.indices.size()
    if anomalous is not None:
        ...
        if group.anomalous_flag:
            ...
            miller_set = miller.set(
                crystal_symmetry=crystal_symmetry, indices=group.indices
            ).auto_anomalous(min_fraction_bijvoet_pairs=2 / 3.0)
    result = miller_set.array(data=group.data, sigmas=sigmas).set_info(
...
class _:
    def crystal_symmetry(self):
        ...
        return cctbx.crystal.symmetry(
            unit_cell=unit_cell, space_group_info=self.mtz_object().space_group_info()
        )
...
class _:
    ...
    def _add_observations(
        self, data_label, sigmas_label, column_types, indices, data, sigmas
    ):
        mtz_reflection_indices = self.add_column(
            label=data_label, type=column_types[0]
        ).set_reals(miller_indices=indices, data=data)
        if sigmas is not None:
            self.add_column(label=sigmas_label, type=column_types[1]).set_reals(
                mtz_reflection_indices=mtz_reflection_indices, data=sigmas
            )

    def _add_complex(self, amplitudes_label, phases_label, column_types, indices, data):
        mtz_reflection_indices = self.add_column(
            label=amplitudes_label, type=column_types[0]
        ).set_reals(miller_indices=indices, data=flex.abs(data))
        self.add_column(label=phases_label, type=column_types[1]).set_reals(
            mtz_reflection_indices=mtz_reflection_indices, data=flex.arg(data, True)
        )

    def add_miller_array(
        self, miller_array, column_root_label, column_types=None, label_decorator=None
    ):
        ...
        if column_types is None:
            ...
            raise RuntimeError("Invalid MTZ column_types for the given miller_array.")
        ...
        if not miller_array.anomalous_flag():
            if default_col_types in ["FQ", "JQ"]:
                ...
                self.add_column(label=column_root_label, type=column_types).set_reals(
                    miller_indices=miller_array.indices(), data=miller_array.data()
                )
            ...
            if default_col_types == "FQDQY":
                ...
                for i, (mi, data) in enumerate(
                    [(hf, f), (hf, sf), (hd, d), (hd, sd), (hf, isym)]
                ):
                    self.add_column(
                        label=label_group[i], type=column_types[i]
                    ).set_reals(miller_indices=mi, data=data)
            else:
                for anomalous_sign in ("+", "-"):
                    ...
                    if default_col_types in ["GL", "KM"]:
                        ...
                        self.add_column(
                            ...
                            type=column_types,
                        ).set_reals(miller_indices=indices, data=data.as_double())
                    elif default_col_types == "AAAA":
                        mtz_reflection_indices = self.add_column(
                            ...
                            type=column_types[0],
                        ).set_reals(miller_indices=indices, data=data.slice(0))
                        for i in range(1, 4):
                            self.add_column(
                                label=label_decorator.hendrickson_lattman(
...
class _:
    def extract_values_and_selection_valid(self, not_a_number_substitute=0):
        return column_values_and_selection_valid(
            values=self.extract_values(not_a_number_substitute=not_a_number_substitute),
    ...
    def format_fields_for_mtz_dump(self, n_refl):
        ...
        fields = [
            self.label(),
            "%d" % valid_values.size(),
            "%.2f%%" % (100.0 * valid_values.size() / max(1, n_refl)),
            format_min_max(flex.min, valid_values),
            format_min_max(flex.max, valid_values),
            self.type() + ":",
            column_type_legend.get(self.type(), "*** UNDEFINED column type ***"),
        ]
        return fields


def miller_array_as_mtz_dataset(
    self,
    column_root_label,
    column_types=None,
    ...
    return (
        mtz_object.add_crystal(
            name=crystal_name, project_name=project_name, unit_cell=unit_cell
        )
        .add_dataset(name=dataset_name, wavelength=wavelength)
        .add_miller_array(
            miller_array=self,
...
class _:
    def show(self, out=None):
        ...
        print(
            "mosaicity model: 0 = isotropic, 1 = anisotropic:", self.lcrflg(), file=out
        )
        print("type of data: 2D (1), 3D (2), or Laue (3):", self.ldtype(), file=out)
        print("goniostat scan axis number:", self.jsaxs(), file=out)
        print(
            "number of batch scales & Bfactors (0 if unset):", self.nbscal(), file=out
        )
  ...
  """
    ...
    for k, other_array in enumerate(cut_arrays[1:], start=1):
        mtz_dataset.add_miller_array(
            other_array, column_root_label=string.ascii_uppercase[k]
        )
