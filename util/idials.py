    ...
    """
        ...
        if short_syntax:
            ...
            working_phil, unused = self.working_phil.peek().fetch(
                source=parse(parameters), track_unused_definitions=True
            )
            if len(unused) > 0:
  ...
  """
    ...
    """
        ...
        if parent is not None:
            ...
            if parent.index >= index:
                raise RuntimeError(
                    "Invalid parent/child indices: %d / %d" % (parent.index, index)
                )
            if parent.workspace is not None and parent.workspace != workspace:
                raise RuntimeError(
                    "Invalid parent/child worksapce: %s / %s"
                    % (parent.workspace, workspace)
                )
            if parent.name not in self.allowed_parents:
                raise RuntimeError(
                    "Invalid parent/child: %s / %s" % (parent.name, self.name)
                )
    ...
    """
        ...
        if exists(self.state.directory):
            raise RuntimeError(
                "Output directory %s already exists" % self.state.directory
            )
  ...
  """
    ...
    """
        ...
        if self.state.parent.name == "index":
            self.filenames = {"input.experiments": self.state.parent.experiments}
        else:
            self.filenames = {"input.datablock": self.state.parent.datablock}
...
class Reindex(Command):
  ...
  """
    ...
    """
        ...
        bs_filenames = RefineBravaisSettings.bravais_setting_filenames(
            self.state.parent
        )
    ...
    """
        memento = cls.Memento(
            ...
            commands=CommandState.from_dict(
                dictionary["commands"], workspace=dictionary["workspace"]
            ),
        )
