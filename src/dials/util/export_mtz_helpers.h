...
namespace dials { namespace util {
  ...
  public:
    BatchArrays(int max_batch_number,
      ...
      // Size arrays appropriately setting all values to zero
      _ints.resize(29 * n_batches, 0);
      _floats.resize(156 * n_batches, 0.0);

      for (int i_batch = 0; i_batch < n_batches; i_batch++) {
        // Index into the _ints array for the start of this batch
        int i = 29 * i_batch;

        // These are the values set by CCP4 programs like COMBAT
        _ints[i + 0] = 29 + 156;
        _ints[i + 1] = 29;
        _ints[i + 2] = 156;
        ...
        _ints[i + 12] = 1;  // ncryst
        // _ints[i + 13] lcrflg (mosaicity model: 0 = isotropic, 1 = anisotropic)
        _ints[i + 14] = 2;  // ldtype 3D
        _ints[i + 15] = 1;  // jsaxs - goniostat scan axis number
        // _ints[i +16] nbscal (number of batch scales & Bfactors)
        _ints[i + 17] = 1;  // ngonax - number of goniostat axes
        // _ints[18] lbmflg (flag for type of beam info: 0 for alambd, delamb; 1
        // also delcor, divhd, divvd)
        _ints[i + 19] = 1;  // ndet
        _ints[i + 20] = dataset_id;

        // Index into the _floats array for the start of this batch
        i = 156 * i_batch;

        // floats[0] to floats[5] is the cell
        for (int j = 0; j < 6; j++) {
          _floats[i + j] = cell_array(i_batch, j);
        }
        // floats[6] to floats[14] is the Umat
        for (int j = 0; j < 9; j++) {
          _floats[i + 6 + j] = umat_array(i_batch, j);
        }
        ...
        _floats[i + 80 + i_min] = -1.0;
        // floats[83] to floats[85] is so the source vector (including tilts)
        for (int j = 0; j < 3; j++) {
          _floats[i + 83 + j] = source[j];
        }
        _floats[i + 86] = wavelength;  // alambd
        ...
        // first
        _floats[i + 111] = panel_distance;

        // floats[113] to floats[120] are detlm (detector limits)
        _floats[i + 113] = panel_size[0];  // NX
        _floats[i + 114] = panel_size[1];  // NY
      }
    }
