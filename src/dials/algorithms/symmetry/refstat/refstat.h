...
namespace cctbx { namespace sgtbx { namespace refstat {
...
class extinction_element {
  ...
  std::vector<const extinction_element*> subs,
    shadowed_by;
  af::shared<rt_mx> rmx;
...
public:
  extinction_element()
    : id(~0), name(), sumI(0), sumS_sq(0), count(0), tag(-1)
  {
    init_mtx();
  }
  ...
  const extinction_element* get_super() const { return super; }
  ...
  extinction_element& set_shadowed_by(extinction_element* e) {
    shadowed_by.insert(shadowed_by.end(),
      e->shadowed_by.begin(), e->shadowed_by.end());
    return *this;
  }
...
private:
  ...
  element_t* screw_21a, * screw_21b, * screw_21c,
    * screw_31, * screw_41, * screw_42, * screw_61,
    * glide_a2, * glide_a3,
    * glide_b1, * glide_b3,
    * glide_c1, * glide_c2,
    * glide_n1, * glide_n2, * glide_n3,
    * glide_d1, * glide_d2, * glide_d3;
...
public:
  ...
  void init() {
    ...
    */
    ...
    elements.push_back(element_t(elements.size(),
      "d--", rt_mx(rot_mx(-1, 0, 0, 0, 1, 0, 0, 0, 1), tr_vec(0, 3, 3))));
    ...
    elements.push_back(element_t(elements.size(),
      "-d-", rt_mx(rot_mx(1, 0, 0, 0, -1, 0, 0, 0, 1), tr_vec(3, 0, 3))));
    ...
    elements.push_back(element_t(elements.size(),
      "--d", rt_mx(rot_mx(1, 0, 0, 0, 1, 0, 0, 0, -1), tr_vec(3, 3, 0))));
    ...
    screw_21c
      ->add_shadowed_by(glide_c1)
      .add_shadowed_by(glide_n1)
      .add_shadowed_by(glide_c2)
      .add_shadowed_by(glide_n2)
      ;

    screw_41->set_shadowed_by(screw_21c)
      .add_shadowed_by(screw_21c)
      .add_shadowed_by(glide_d1);

    screw_42->set_shadowed_by(screw_21c)
      .add_shadowed_by(screw_21c);
  ...
  void process(const af::shared<miller::index<> >& indices,
    const af::shared<FloatType>& Is, const af::shared<FloatType>& sigs,
    FloatType scale_ = 0)
  {
  ...
  void process_omp(const af::shared<miller::index<> >& indices,
    const af::shared<FloatType>& Is, const af::shared<FloatType>& sigs,
    FloatType scale_ = 0, int th_num = -1)
  {
    gather_stats(Is, sigs, scale_);
    if (th_num < 0) {
...
#pragma omp parallel for num_threads(th_num)
  ...
  bool has_openmp() const { return true; }
#else
  void process_omp(const af::shared<miller::index<> >& indices,
    const af::shared<FloatType>& Is, const af::shared<FloatType>& sigs,
    FloatType scale_ = 0, int th_num = -1)
  ...
  bool has_openmp() const { return false; }
#endif
...
protected:
  ...
  void process_1(const miller::index<>& h, const FloatType& I, const FloatType& s_sq) {
    if (h[0] == 0) {
      if ((h[1] % 2) != 0) {
  ...
  void gather_stats(const af::shared<FloatType>& Is,
    const af::shared<FloatType>& sigs, FloatType scale_ = 0)
  {
    reset_this();
    ref_count = Is.size();
