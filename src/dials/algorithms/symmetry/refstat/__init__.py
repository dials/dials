...
class registry(extinctions_registry):
  ...
  def show_extinctions_for(self, sg_name):
    ...
    print("Extinction elements for %s: %s"\
      %(sg_name, " ".join([self.__getitem__(i).name for i in self.get_extinctions(sg)])))
...
class extinctions(extinctions_registry):
  ...
  def analyse(self, scale_I_to=None):
    ...
    if scale_I_to is None:
      scale_I_to = 0.0
    if self.has_omp:
      self.process_omp(self.miller_array.indices(),
         self.miller_array.data(), self.miller_array.sigmas(), scale=scale_I_to)
    else:
      self.process(self.miller_array.indices(),
         self.miller_array.data(), self.miller_array.sigmas(), scale=scale_I_to)
  ...
  def print_stats(self):
    ...
    for x in self.all_elements:
      ...
      print("%-4s (%5s): %16.2f(%6.2f) %s" %(
        x.name, x.count, x.meanI, x.sig, flag))
  ...
  def get_filtered_matching_space_groups(self, matches=None, cell_compatible_only=True):
    if matches is None:
    ...
    for sg, mp in matches:
      if cell_compatible_only and not sg.is_compatible_unit_cell(uc):
        continue
      mt = merge_test(self.miller_array.indices(),
        self.miller_array.data(), self.miller_array.sigmas())
