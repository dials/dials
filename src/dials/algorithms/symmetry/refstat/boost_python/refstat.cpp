...
namespace {
  struct nsp_wrapper {
    ...
    static void wrap() {
      ...
      class_<wt, bases<space_group>,
        std::auto_ptr<wt> >("named_space_group", no_init)
        .def(init<const space_group_symbols&>(
          (arg("space_group_symbols"))))
        .add_property("name", make_function(&wt::get_name, rbv))
        .def("contains", &wt::contains_all)
        ;
      scitbx::af::boost_python::shared_wrapper<wt, rir_t>::wrap("shared_named_space_groups");
    }
  };
  ...
  struct sar_wrapper {
    ...
    static void wrap() {
      ...
      class_<wt>("extinctions_registry", no_init)
        ...
        .def("process", &wt::process,
          ((arg("indices"), arg("Is"), arg("sigs"), arg("scale") = 0)))
        .def("process_omp", &wt::process_omp,
          ((arg("indices"), arg("Is"), arg("sigs"), arg("scale") = 0,
            arg("threads_n") = -1)))
        .def("reset", &wt::reset)
      ...
      static void wrap() {
        ...
        class_<wt>("merge_test", no_init)
          .def(init<const af::shared<miller::index<> >&,
            ...
            const af::shared<FloatType>&>((arg("indices"), arg("Is"), arg("sigs"))))
