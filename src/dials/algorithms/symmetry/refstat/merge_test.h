  ...
  sysabs_test_result sysabs_test(const space_group& sg, FloatType scale=1.0) const {
    ...
    for (size_t i = 0; i < indices.size(); i++) {
      for (size_t j = 1; j < sg.order_z(); j++) {
        ...
        FloatType ps = rt.t().as_floating_point<FloatType>(scitbx::type_holder<FloatType>())
          * indices[i];
...
protected:
  struct dry_shelx_merger {
    ...
    dry_shelx_merger()
      : inconsistent_eq(0), r_int_num(0), r_int_den(0)
    {}
        ...
        af::const_ref<FloatType> const& unmerged_sigmas)
    {
      ...
      for (size_t i = 0; i < n; i++) {
        ...
        const FloatType s = std::max(static_cast<FloatType>(1e-5),
          unmerged_sigmas[index]);
      ...
      if (n > 1) {
        ...
        r_int_den += sum_i;
        const FloatType
          sig_int = sum_diff / (n * sqrt(static_cast<FloatType>(n) - 1.0));
        if (sig_int > sig) {
          if (sig_int > 5 * sig) {
            inconsistent_eq++;
          }
