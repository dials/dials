...
def find_elbow(x: np.ndarray, y: np.ndarray) -> float:
    """Identify an elbow point in a curve
    ...
    x1 = np.array((x[p_m], y[p_m]))
    x2 = np.array((x[-1], y[-1]))

    gaps = []
    v = np.array(((x2[1] - x1[1]), -(x2[0] - x1[0])))
    v /= np.linalg.norm(v)
    ...
    def _determine_dimensions(self):
        if self.params.dimensions is Auto and self.target.dim == 2:
            ...
            for dim in range(1, self.target.dim + 1):
                ...
                if dim > 2:
                    x_g = find_elbow(np.array(dimensions), np.array(functional))
                    n_flat_points = dim - x_g
                    logger.debug("x_g: %s, n_flat_points: %s", x_g, n_flat_points)
                    if n_flat_points >= 2:
