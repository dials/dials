...
class Indexer:
    ...
    def refine_cell_rotation(self, H, scattering_vector, fix="a"):
        ...
        fix_mask = [(i in fix) for i in cell_param_names.split(',')]
    ...
    def index_pink(self, s0, s1, max_refls=50, rotogram_grid_points=200, voxel_grid_points=200):
        """
        Args:
        ...
        Hall = generate_reciprocal_cell(self.cell, dmin, dtype='float32')
        h = (self.B @ Hall.T).T
        hhat = normalize(h)
        ...
        phi = np.linspace(-2*np.pi, 0., rotogram_grid_points)
        c1 = np.sin(phi / 2.)
        c2 = -np.cos(phi / 2.)
        ...
        discretized = _discretized
        if window_size is not None:
            # Restrict search to a region around self.R
            v_init = rotation_to_crv(Rotation.from_matrix(self.R))
            diff = v - v_init
            sel = norm2(diff) <= window_size * rad / v_grid_size
            discretized = discretized[sel]
        discretized = discretized.reshape((-1, 3))
        ...
        Hout[Hidx] = Hobs
        q = self.s1[Hidx] - self.s0[None,:]

        vnorm = norm2(v_max)
        theta = 4. * np.tan(vnorm)
        axis = v_max / vnorm
...
class PinkIndexer(Strategy):
    ...
    def __init__(
        ...
        self._params.pink_indexer.cell = (
            target_symmetry_primitive.unit_cell().parameters()
        )
        self._params.pink_indexer.spacegroup = str(
