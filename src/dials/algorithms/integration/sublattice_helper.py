...
def integrate_coset(self, experiments, indexed):
    ...
    triclinic = base_set.customized_copy(
        crystal_symmetry=symmetry(
            unit_cell=experiments_local[0].crystal.get_unit_cell(), space_group="P1"
        )
    )
    ...
    experiments_local = ProfileModelFactory.create(
        self.params, experiments_local, indexed
    )
