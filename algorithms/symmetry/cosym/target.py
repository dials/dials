...
class Target(object):
    ...
    def plot_wij_matrix(self, plot_name=None):
        ...
        plt.clf()
        plt.imshow(
            self.wij_matrix.as_numpy_array(), vmin=0, vmax=flex.max(self.wij_matrix)
        )
        plt.colorbar()
        if plot_name is not None:
            plt.savefig(plot_name)
    ...
    def plot_wij_cumulative_frequency(self, plot_name=None):
        ...
        non_zero_sel = wij > 0
        logger.info(
            "%i (%.1f%%) non-zero elements of Wij matrix"
            % (
                non_zero_sel.count(True),
                100 * non_zero_sel.count(True) / non_zero_sel.size(),
            )
        )
