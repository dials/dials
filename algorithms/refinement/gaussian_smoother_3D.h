...
namespace dials { namespace refinement {
  ...
  public:
    ...
    GaussianSmoother3D(vec2<double> x_range, std::size_t num_x_intervals,
      vec2<double> y_range, std::size_t num_y_intervals,
      vec2<double> z_range, std::size_t num_z_intervals)
     ...
     */
    SingleValueWeights value_weight(double x, double y, double z,
        ...
        for (int j = jrange[0]; j < jrange[1]; ++j) {
          for (int k = krange[0]; k < krange[1]; ++k) {
            double ds = pow(pow(z1 - x_positions_[i], 2)
                            + pow(z2 - y_positions_[j], 2)
                            + pow(z3 - z_positions_[k], 2), 0.5) / sigma_;
    ...
    MultiValueWeights multi_value_weight(const af::const_ref<double> x,
      const af::const_ref<double> y, const af::const_ref<double> z,
      const af::const_ref<double> values) {
          ...
          for (int jcol = jrange[0]; jcol < jrange[1]; ++jcol) {
            for (int kcol = krange[0]; kcol < krange[1]; ++kcol) {
              double ds = pow(pow(z1 - x_positions_[icol], 2)
                              + pow(z2 - y_positions_[jcol], 2)
                              + pow(z3 - z_positions_[kcol], 2), 0.5) / sigma_;
              double w = exp(-ds*ds);
