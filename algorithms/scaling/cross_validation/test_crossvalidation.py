...
def test_crossvalidator():
  ...
  assert len(crossvalidator.results_dict[0].keys()) == \
  ...
  assert len(crossvalidator.results_dict[0].keys()) == \
    len(crossvalidator.results_metadata['names']) + 1
  assert len(crossvalidator.results_dict[1].keys()) == \
    len(crossvalidator.results_metadata['names']) + 1
...
def test_cross_validate_script():
  ...
  with mock.patch(fpath + 'crossvalidator.DialsScaleCrossValidator.run_script') as \
    mock_run_script:
    with mock.patch(fpath + 'crossvalidator.DialsScaleCrossValidator.interpret_results') as \
      mock_interpret:
      cross_validate(param, crossvalidator)
