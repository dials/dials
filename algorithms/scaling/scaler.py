...
class SingleScaler(ScalerBase):
    ...
    def combine_intensities(self, use_Imid=None):
        ...
        try:
            ...
            self.experiments.scaling_model.record_intensity_combination_Imid(
                combiner.max_key
            )
...
class MultiScalerBase(ScalerBase):
    ...
    def adjust_variances(self):
        ...
        if self.verbosity <= 1:
            if (
                self.single_scalers[0].experiments.scaling_model.error_model
        ...
        if self.params.reflection_selection.combine.joint_analysis:
            ...
            try:
                ...
                for i, scaler in enumerate(self.active_scalers):
                    ...
                    scaler.experiments.scaling_model.record_intensity_combination_Imid(
                        combiner.max_key
                    )
        else:
