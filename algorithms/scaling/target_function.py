...
class target_function(object):
  ...
  def calculate_residual(self):
    ...
    weights = self.data_manager.sorted_reflections['scaling_weightings']
...
class xds_target_function(target_function):
  ...
  def calculate_gradient(self):
    ...
    dIhdg = flex.double((intensities - (Ih_values * 2.0 * scale_factors)) 
                         / (variances * sumgsq))
    rhl = flex.double((intensities - (Ih_values
                      * scale_factors)
              ) / (variances**0.5))
    grad = ((2.0 * rhl * ((-1.0 * Ih_values / (variances**0.5))
              - ((scale_factors / (variances**0.5)) * dIhdg)))
              + (2.0 * (scale_factors - 1.0)/(sigma**2)))
    grad_selection = grad * weights
    return flex.double(np.bincount(self.data_manager.sorted_reflections[
      self.data_manager.active_bin_index], weights=grad_selection, 
