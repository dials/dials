...
namespace dials { namespace algorithms { namespace shoebox {
  ...
  public:
    ...
    /**
     * Set all the foreground/background pixels in the shoebox mask by looking at the nearest bright spots
     * @param table Reflection table with a shoebox array and a bbox array for masking
     */
    void mask(af::reflection_table &table) {
      // Convert the reference xyz mm values to a single array for AnnAdaptor, instead of a vec3 double array
      ...
      for (std::size_t iter=0; iter < reference_xyz_mm.size(); ++iter){
        ...
        reference_xyz_mm_values.push_back(item[0]); reference_xyz_mm_values.push_back(item[1]); reference_xyz_mm_values.push_back(item[2]);
      }

      // Convert the query xyz mm values to a single array for AnnAdaptor, instead of a vec3 double array
      ...
      for (std::size_t iter=0; iter < query_xyz_mm.size(); ++iter){
        ...
        query_xyz_mm_values.push_back(item[0]); query_xyz_mm_values.push_back(item[1]); query_xyz_mm_values.push_back(item[2]);
      }
      ...
      af::shared<Shoebox <> > reference_shoeboxes = reference_.get< Shoebox <> >("shoebox");
      ...
      for (std::size_t query_iter=0; query_iter < table.size(); ++query_iter) {
        ...
        int tblx1 = table_bbox[0]; int tbly1 = table_bbox[2]; int tblz1 = table_bbox[4];
        ...
        for (std::size_t nn_iter = query_iter * nn_window; nn_iter < (query_iter * nn_window) + nn_window; ++nn_iter) {
          ...
          int nnx1 = bbox_nn[0]; int nny1 = bbox_nn[2]; int nnz1 = bbox_nn[4];
          int nnx2 = bbox_nn[1]; int nny2 = bbox_nn[3]; int nnz2 = bbox_nn[5];
            ...
            for (std::size_t y = 0; y < nny2-nny1; y++) {
              for (std::size_t x = 0; x < nnx2-nnx1; x++) {
                table_mask(
                  tmid_z-rmid_z+z,
                  tmid_y-rmid_y+y,
                  tmid_x-rmid_x+x) |= reference_mask(z,y,x);
              }
            }
          }
        }

        // Finally, need to explicitly set the background flags for the rest of the pixels as they are not set by spotfinder in
        // the reference set
