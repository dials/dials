...
namespace boost_python {
  ...
  struct compare<Box3d, boost_python::OctreeObject<Object> > {
    ...
    static
    bool contains(const Box3d &box, const boost_python::OctreeObject<Object> &v) {
      return box.x0 <= v.object[0] && box.y0 <= v.object[1] && box.z0 <= v.object[2]
          && box.x1 >= v.object[0] && box.y1 >= v.object[1] && box.z1 >= v.object[2];
    }

    static
    bool collides(const Box3d &box, const boost_python::OctreeObject<Object> &v) {
...
namespace boost_python {
  ...
  template <typename ObjectType>
  Octree< OctreeObject<ObjectType> > *make_octree(
      af::int6 box, int max_bucket_size) {
    return new Octree< OctreeObject<ObjectType> >(
      Box3d(box[0], box[2], box[4], box[1], box[3], box[5]),
      max_bucket_size);
  }
  ...
  af::shared<std::size_t> octree_query_range(
    ...
    bool success = tree.query_range(
      Box3d(box[0], box[2], box[4], box[1], box[3], box[5]), temp);
  ...
  template <typename ObjectType>
  Octree< OctreeObject<ObjectType> > *make_octree_from_list(
      af::const_ref<ObjectType> a,
      int max_bucket_size) {
