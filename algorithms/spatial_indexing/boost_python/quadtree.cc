...
namespace boost_python {
  ...
  struct compare<Box, boost_python::QuadtreeObject<Object> > {
    ...
    static
    bool contains(const Box &box, const boost_python::QuadtreeObject<Object> &v) {
      return box.x0 <= v.object[0] && box.y0 <= v.object[1]
          && box.x1 >= v.object[0] && box.y1 >= v.object[1];
    }

    static
    bool collides(const Box &box, const boost_python::QuadtreeObject<Object> &v) {
...
namespace boost_python {
  ...
  template <typename ObjectType>
  Quadtree< QuadtreeObject<ObjectType> > *make_quadtree(
      af::int4 box, int max_bucket_size) {
  ...
  template <typename ObjectType>
  Quadtree< QuadtreeObject<ObjectType> > *make_quadtree_from_list(
