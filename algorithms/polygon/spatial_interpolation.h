...
namespace dials { namespace algorithms { namespace polygon {
   ...
   */
  inline
  double reverse_quad_inplace_if_backward(vert4 &input) {
   ...
   */
  inline
  double quad_grid_intersection_area(const vert4 &a, int i, int j) {
    vert4 b(vec2<double>(i, j), vec2<double>(i+1, j),
            vec2<double>(i+1, j+1), vec2<double>(i, j+1));
   ...
   */
  inline
  af::versa< double, af::c_grid<2> > regrid_grid_to_irregular_grid(
      const af::const_ref< double, af::c_grid<2> > &input,
      const af::const_ref< vec2<double>, af::c_grid<2> > &outputxy) {
