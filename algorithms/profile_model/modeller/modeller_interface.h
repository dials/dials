...
namespace dials { namespace algorithms {
  ...
  public:
    ...
    virtual
    void model(af::reflection_table) = 0;

    virtual
    void accumulate(boost::shared_ptr<ProfileModellerIface>) = 0;

    virtual
    void finalize() = 0;

    virtual
    bool finalized() const = 0;

    virtual
    data_type data(std::size_t) const = 0;

    virtual
    mask_type mask(std::size_t) const = 0;
