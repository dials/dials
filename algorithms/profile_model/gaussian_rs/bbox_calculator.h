...
namespace dials { namespace algorithms { namespace shoebox {
  ...
  public:
     ...
     */
    double operator()(vec3 <double> s1, double phi, int6 bbox) const {
    ...
    af::shared<double> operator()(
      ...
      for (std::size_t i = 0; i < s1.size(); ++i) {
        result[i] = operator()(s1[i], phi[i], bbox[i]);
      }
      return result;
    }
  ...
  public:
     ...
     * Add a bbox calculator to the list.
     */
    void push_back(const PartialityCalculator &obj) {
      compute_.push_back(obj);
    }
    ...
    af::shared<double> operator()(
      ...
      for (std::size_t i = 0; i < s1.size(); ++i) {
        DIALS_ASSERT(id[i] < size());
        result[i] = compute_[id[i]](s1[i], phi[i], bbox[i]);
      }
      return result;
    }
  ...
  private:
    ...
    std::vector<PartialityCalculator> compute_;
  };
