...
def test_reflections(file_base):
    ...
    assert(cs != None)
    print("Original space group: %s" %(cs.space_group().match_tabulated_settings().hermann_mauguin()))
    ...
    for root, dirs, files in os.walk(root_):
        for f in files:
            ...
            try:
                ...
                matches, filtred_matches = get_matches(cs, hkl_path, original_sg_name[0])
                if matches is None:
