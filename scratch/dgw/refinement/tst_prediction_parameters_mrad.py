...
for iref in selection:
    ...
    for i, (a, b) in enumerate(zip(an_grads, fd_grads)):
        try:
            assert approx_equal(a - b, matrix.col((0., 0., 0.)),
                                eps = 1.e-6)
        except AssertionError:
            print "Failure for parameter number %d" %i
            print "Analytical derivatives: %.6f, %.6f, %.6f" % a.elems
            print "Finite derivatives: %.6f, %.6f, %.6f" % b.elems
            raise
