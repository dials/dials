...
#    paintmask = numpy.zeros( n_x * n_y, dtype = int ).reshape( n_x, n_y )
    ...
    for times in range( 5 ):
        for pos in range( len( xcoord ) ):
            for x in range( int( xcoord[pos] ) - 5, int( xcoord[pos] + 6 ), 1 ):
                for y in range( int( ycoord[pos] ) - 5, int( ycoord[pos] ) + 6, 1 ):
    ...
    single_spot_integrate( data2d, xcoord, ycoord , cntrd_xcoord, cntrd_ycoord, x_sigma, y_sigma, diffdata2d_ext )
...
#    plt.show()
#
def single_spot_integrate( data2d, xcoord, ycoord , cntrd_xcoord, cntrd_ycoord, x_sigma, y_sigma, diffdata2d_ext ):
    for pos in range( len( xcoord ) ):
        x_num_sum = 0.0
        y_num_sum = 0.0
        den_sum = 0.0
