...
def any_compatible_unit_cell(space_group, volume=None, asu_volume=None):
    ...
    f = (volume / unit_cell.volume()) ** (1 / 3.0)
...
def generate_spots(
    crystal_model, detector, beam, goniometer=None, scan=None, sel_fraction=1.0
):
    ...
    sel = flex.random_selection(
        len(predicted), int(math.floor(sel_fraction * len(predicted)))
    )
...
def add_random_noise_xyz(experiments, strong_spots, rmsds):
    errors = flex.vec3_double(
        *list(
            scitbx.random.variate(scitbx.random.normal_distribution(0, rmsds[i]))(
                len(strong_spots)
            )
            for i in range(3)
        )
...
@pytest.mark.parametrize(
    ("space_group", "unit_cell_volume"),
    [("P1", 1000), ("P2", 10000), ("P222", 10000), ("P23", 10000)],
)
def test_index_synthetic(
    space_group, unit_cell_volume, dials_regression, run_in_tmpdir
):
    space_group = sgtbx.space_group_info(symbol=space_group).group()
    unit_cell = any_compatible_unit_cell(space_group, volume=unit_cell_volume)
