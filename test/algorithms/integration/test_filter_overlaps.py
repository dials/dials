  ...
  for expt, refl in zip(experiments, reflections):
    ...
    for obs in refl:
      ...
      for f, s in zip(fast_coords, slow_coords):
        ...
        assert not (is_fgd(shoebox.mask[posn_in_shoebox]) and is_fgd(mask_array[posn])), \
          "Overlapping foreground found at indexed position (%d, %d), " % (f_abs, s_abs) \
    ...
    for i, this_code in enumerate(mask_array):
      assert not is_overlap(this_code), \
          "Overlapping foreground and background found at (%d, %d)" % (i % shoebox.xsize(), i // shoebox.xsize())
