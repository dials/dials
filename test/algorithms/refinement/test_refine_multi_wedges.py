...
def test_order_invariance(dials_regression, run_in_tmpdir):
  ...
  result = procrunner.run_process([
      ...
      "experiments={0}/data/sweep_%03d/experiments.json".format(data_dir) % n for n in selection1
  ] + [
      "reflections={0}/data/sweep_%03d/reflections.pickle".format(data_dir) % n for n in selection1
  ])
  assert result['exitcode'] == 0
  assert result['stderr'] == ''
  result = procrunner.run_process([
  ...
  result = procrunner.run_process([
      ...
      "experiments={0}/data/sweep_%03d/experiments.json".format(data_dir) % n for n in selection2
  ] + [
      "reflections={0}/data/sweep_%03d/reflections.pickle".format(data_dir) % n for n in selection2
  ])
  assert result['exitcode'] == 0
  assert result['stderr'] == ''
  result = procrunner.run_process([
  ...
  # Load results
  refined_experiments1 = ExperimentListFactory.from_json_file("refined_experiments1.json", check_format=False)
  refined_experiments2 = ExperimentListFactory.from_json_file("refined_experiments2.json", check_format=False)
