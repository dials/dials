...
def test_ScanVaryingCrystalOrientationParameterisation_intervals(nintervals, plots=False):
...
def test_ScanVaryingCrystalOrientationParameterisation_random(plots=False):
  ...
  for i in range(attempts):
    ...
    a = random.uniform(10,50) * \
            vmp.random_direction_close_to(matrix.col((1, 0, 0)))
    b = random.uniform(10,50) * \
            vmp.random_direction_close_to(matrix.col((0, 1, 0)))
    c = random.uniform(10,50) * \
            vmp.random_direction_close_to(matrix.col((0, 0, 1)))
    ...
    xl_op_fd_ds_dp = get_fd_gradients(xl_op, [1.e-6 * math.pi/180] * \
                                      num_param)

    for j in range(num_param):
      assert approx_equal((xl_op_fd_ds_dp[j] - xl_op_an_ds_dp[j]),
                          null_mat, eps = 1.e-6), textwrap.dedent("""\
        Failure in try {i}
...
if __name__ == '__main__':
  ...
  for intervals in (1,2,3,4,5,6,7):
    test_ScanVaryingCrystalOrientationParameterisation_intervals(intervals, plots=False)
  ...
  for intervals in (1,2,3,4,5,6,7):
    test_ScanVaryingCrystalUnitCellParameterisation_intervals(intervals, plots=False)
