...
def test_refinement_and_compare_with_known_truth(dials_regression, tmpdir):
  tmpdir.chdir()
  ...
  sim_s0_scan_points = [sim.beam.get_s0_at_scan_point(i) for i in range(
      sim.beam.num_scan_points)]
  sim_bc_scan_points = [sim.detector[0].get_beam_centre_px(s0)
                        for s0 in sim_s0_scan_points]
