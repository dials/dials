...
def create_experiments(image_start=1):
  ...
  sf = ScanFactory()
  scan = sf.make_scan(image_range = (image_start, image_start + 720 - 1),
                        exposure_times = 0.1,
                        oscillation = (0, 0.1),
                        epochs = range(720),
                        deg = True)
  ...
  experiments.append(Experiment(
        beam=beam, detector=detector, goniometer=goniometer, scan=scan,
        crystal=crystal, imageset=None))
...
def test_per_width_and_per_image_are_equivalent():
  ...
  phi_obs = reflections['xyzobs.mm.value'].parts()[2] * 180./pi
