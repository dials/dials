...
def test_regression(data):
  ...
  assert r_new[0]['xyzcal.px'] == pytest.approx((75.33831543451907, 2327.55737978813, 0.2548567552525226), abs=1e-6)
...
def test_scan_varying_results_are_close_to_static_prediction_when_model_is_static(dials_regression):
  ...
  from dials.test.algorithms.spot_prediction.test_scan_static_reflection_predictor import Data as StaticTest
...
def test_for_reflection_table(data):
  ...
  preds['ub_matrix'] = flex.mat3_double(len(preds), data.experiments[0].crystal.get_A())
  ...
  preds['S_matrix'] = flex.mat3_double(len(preds),
      data.experiments[0].goniometer.get_setting_rotation())
