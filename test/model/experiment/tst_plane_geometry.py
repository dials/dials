...
def tst_intersection_at_corners(intersection, detector, wavelength):

    from scitbx import matrix
    eps = 1e-7

    # Get parameters from detector
    origin = matrix.col(detector.origin)
    fast_axis = matrix.col(detector.fast_axis)
    slow_axis = matrix.col(detector.slow_axis)
...
def tst_intersection_away_from_detector(intersection, detector, wavelength):

    from scitbx import matrix

    # Set the beam vector to directed away from the detector origin
    s = - (1.0 / wavelength) * matrix.col(detector.origin)
...
def tst_beam_plane_intersection():
    from dials.model.experiment import BeamPlaneIntersection
    ...
    detector = Detector("", fast_axis, slow_axis, origin, pixel_size,
                        image_size, (0, 0))

    # Create the intersection object
    intersection = BeamPlaneIntersection(detector.get_D_matrix())

    # Perform a load of tests
    tst_intersection_at_origin(intersection, wavelength, origin)
    tst_intersection_at_corners(intersection, detector, wavelength)
    tst_intersection_away_from_detector(intersection, detector, wavelength)

def tst_transform_at_origin(transform, detector):
    ...
    assert(abs(x - detector.origin[0]) < eps)
    assert(abs(y - detector.origin[1]) < eps)
    assert(abs(z - detector.origin[2]) < eps)

    # Test Passed
    print "OK"

def tst_transform_at_corners(transform, detector):
    from scitbx import matrix
    eps = 1e-7
...
def tst_plane_to_lab_transform():
    from dials.model.experiment import PlaneToLabTransform
    from dials.model.experiment import Detector
    from scitbx import matrix
    ...
    # Create the intersection object
    transform = PlaneToLabTransform(detector.get_d_matrix())

    # Perform a load of tests
    tst_transform_at_origin(transform, detector)
    tst_transform_at_corners(transform, detector)

def tst_fwd_rev_random(intersection, transform, detector):
    from scitbx import matrix
...
def tst_forward_and_reverse_transform():
    ...
    # Do a test
    tst_fwd_rev_random(intersection, transform, detector)
