...
def test_sort_intensities(dials_data, tmpdir):
    result = procrunner.run(
        [
            ...
            "output=sorted1.mpack",
        ],
        working_directory=tmpdir.strpath,
    )
    assert result["exitcode"] == 0
    assert result["stderr"] == ""
    assert tmpdir.join("sorted1.mpack").check(file=1)

    from dials.array_family import flex

    data = flex.reflection_table.from_msgpack_file(tmpdir.join("sorted1.mpack").strpath)
...
def test_reverse_sort_intensities(dials_data, tmpdir):
    ...
    assert result["exitcode"] == 0
    assert result["stderr"] == ""
    assert tmpdir.join("sorted2.mpack").check(file=1)

    from dials.array_family import flex

    data = flex.reflection_table.from_msgpack_file(tmpdir.join("sorted2.mpack").strpath)
...
def test_default_sort_on_miller_index(dials_data, tmpdir):
    result = procrunner.run(
        [
            "dev.dials.sort_reflections",
            dials_data("centroid_test_data").join("integrated.pickle").strpath,
            "output=sorted3.mpack",
        ],
        working_directory=tmpdir.strpath,
    )
    assert result["exitcode"] == 0
    assert result["stderr"] == ""
    assert tmpdir.join("sorted3.mpack").check(file=1)

    from dials.array_family import flex

    data = flex.reflection_table.from_msgpack_file(tmpdir.join("sorted3.mpack").strpath)
