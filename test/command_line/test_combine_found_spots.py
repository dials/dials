...
def test_combining_spots(dials_regression, run_in_tmpdir):
    ...
    result = run_process(
        ["dials.import"] + images_1 + ["output.datablock=datablock-1.json"]
    )
    ...
    result = run_process(
        ["dials.find_spots", "datablock-1.json", "output.reflections=strong-1.pickle"]
    )
    assert result["exitcode"] == 0
    assert result["stderr"] == ""
    ...
    result = run_process(
        ["dials.import"] + images_2 + ["output.datablock=datablock-2.json"]
    )
    assert result["exitcode"] == 0
    assert result["stderr"] == ""
    ...
    result = run_process(
        ["dials.find_spots", "datablock-2.json", "output.reflections=strong-2.pickle"]
    )
    assert result["exitcode"] == 0
    assert result["stderr"] == ""
