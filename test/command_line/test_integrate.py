...
def test2(dials_regression, tmpdir):
  ...
  for i in range(1, 10):
    shutil.copyfile(os.path.join(dials_regression, "centroid_test_data", "centroid_000%d.cbf" % i),
                    "centroid_001%d.cbf" % i)

  with open(os.path.join(dials_regression, "centroid_test_data", "experiments.json"), "r") as fh:
    j = json.load(fh)
...
def test_integrate_with_kapton(dials_regression, tmpdir):
  ...
  pickle_path = os.path.join(dials_regression, "integration_test_data", "stills_PSII", pickle_name)
  json_path = os.path.join(dials_regression, "integration_test_data", "stills_PSII", json_name)
  image_path = os.path.join(dials_regression, "integration_test_data", "stills_PSII", image_name)
