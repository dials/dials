...
def test_multiple_sweep_import_fails_without_allow_parameter(
    ...
    result = run_process(
        ["dials.import"]
        + image_files
        + ["output.datablock=datablock_multiple_sweeps.json"]
    )
...
def test_with_mask(dials_regression, run_in_tmpdir):
    ...
    result = run_process(
        ["dials.import"]
        + image_files
        + ["mask=" + mask_filename, "output.datablock=datablock_with_mask.json"]
    )
    assert result["exitcode"] == 0
    assert result["stderr"] == ""
...
def test_override_geometry(dials_regression, run_in_tmpdir):
  ...
  """
    ...
    result = run_process(
        ["dials.import"]
        + image_files
        + ["geometry.phil", "output.datablock=override_geometry.json"]
    )
    assert result["exitcode"] == 0
    assert result["stderr"] == ""
...
def tst_import_beam_centre(dials_regression, run_in_tmpdir):
    ...
    result = run_process(
        [
            ...
            "output.datablock=mosflm_beam_centre.json",
        ]
        + image_files
    )
    assert result["exitcode"] == 0
    ...
    result = run_process(
        [
            ...
            "output.datablock=mosflm_beam_centre2.json",
        ]
        + image_files
    )
    assert result["exitcode"] == 0
...
def test_slow_fast_beam_centre(dials_regression, run_in_tmpdir):
    ...
    result = run_process(
        [
            ...
            "output.datablock=slow_fast_beam_centre.json",
            impath,
        ]
    )
    assert result["exitcode"] == 0
...
def test_from_image_files(dials_regression, run_in_tmpdir):
    ...
    result = run_process(
        ["dials.import"] + image_files + ["output.datablock=import_datablock.json"]
    )
    assert result["exitcode"] == 0
...
def test_from_template(dials_regression, run_in_tmpdir):
    ...
    result = run_process(
        [
            "dials.import",
            "template=" + template,
            "output.datablock=import_datablock.json",
        ]
    )
    assert result["exitcode"] == 0
    assert os.path.exists("import_datablock.json")
...
def test_extrapolate_scan(dials_regression, run_in_tmpdir):
    ...
    result = run_process(
        [
            "dials.import",
            image,
            "output.datablock=import_extrapolate.json",
