...
def run():
  ...
  assert os.path.exists('reflections_reindexed.pickle')
  new_reflections = easy_pickle.load('reflections_reindexed.pickle')
  old_experiments = load.experiment_list(experiments_path, check_format=False)
  assert os.path.exists('experiments_reindexed.json')
  new_experiments = load.experiment_list(
    'experiments_reindexed.json', check_format=False)
