...
def test_dynamic_shadowing(dials_regression):
    ...
    exercise_one_image(
        path=os.path.join(
            dials_regression, "shadow_test_data/DLS_I03_SmarGon/protk_2_0600.cbf"
        ),
...
def test_shadow_plot(dials_regression, run_in_tmpdir):
    ...
    result = fully_buffered(
        "dials.shadow_plot datablock.json json=shadow.json"
    ).raise_if_errors()
    ...
    result = fully_buffered(
        "dials.shadow_plot datablock.json mode=2d plot=shadow_2d.png"
    ).raise_if_errors()
