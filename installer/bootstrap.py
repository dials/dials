...
class PhaserBuilder(CCIBuilder):
    ...
    def add_tests(self):
        self.add_test_command(
            "phaser_regression.regression",
            args=["all", "-o", "terse_failed", "-n", "%s" % self.nproc],
        )
    ...
    def add_base(self, extra_opts=[]):
        ...
        if self.skip_base is None or len(self.skip_base) == 0:
            self.skip_base = (
                "hdf5,lz4_plugin,py2app,wxpython,docutils,pyopengl,pillow,tiff,"
        ...
        super(PhaserBuilder, self).add_base(extra_opts=["--cctbx"] + extra_opts)
...
class PhaserTNGBuilder(PhaserBuilder):
    ...
    def add_tests(self):
        self.add_test_command(
            "libtbx.import_all_python", workdir=["modules", "cctbx_project"]
        )
...
class CCTBXLiteBuilder(CCIBuilder):
    ...
    # Configure for these cctbx packages
    LIBTBX = ["cctbx", "scitbx", "libtbx", "iotbx", "mmtbx", "smtbx", "gltbx", "wxtbx"]
    ...
    def add_base(self, extra_opts=[]):
        if self.skip_base is None or len(self.skip_base) == 0:
            ...
            self.skip_base = ",".join(
                self.skip_base.split(",") + ["hdf5", "lz4_plugin", "h5py"]
            )
        super(CCTBXLiteBuilder, self).add_base(extra_opts=["--cctbx"] + extra_opts)
...
class CCTBXBuilder(CCIBuilder):
    ...
    def add_base(self, extra_opts=[]):
        super(CCTBXBuilder, self).add_base(extra_opts=["--cctbx"] + extra_opts)
...
class LABELITBuilder(CCIBuilder):
    ...
    def add_base(self, extra_opts=[]):
        super(LABELITBuilder, self).add_base(extra_opts=["--labelit"] + extra_opts)
...
class XFELLegacyBuilder(CCIBuilder):
    CODEBASES_EXTRA = ["dials", "labelit", "cxi_xdr_xes"]
    LIBTBX_EXTRA = ["dials", "labelit", "xfel", "cxi_xdr_xes", "prime", "iota"]
    HOT_EXTRA = ["msgpack"]
...
class XFELBuilder(CCIBuilder):
    CODEBASES_EXTRA = ["dials", "uc_metrics", "ncdist"]
    LIBTBX_EXTRA = ["dials", "xfel", "prime", "iota", "uc_metrics"]
    HOT_EXTRA = ["msgpack"]

    def add_base(self, extra_opts=[]):
        super(XFELBuilder, self).add_base(extra_opts=["--dials"] + extra_opts)
...
class PhenixBuilder(CCIBuilder):
    ...
    def add_base(self, extra_opts=[]):
        super(PhenixBuilder, self).add_base(
            extra_opts=["--phenix", "--labelit", "--dials", "--xia2"] + extra_opts
        )
    ...
    def add_tests(self):
        ...
        self.rebuild_docs()
        self.add_test_command(
            "phenix_regression.run_p9_sad_benchmark", name="test p9 sad"
        )
        self.add_test_command(
            "phenix_regression.run_hipip_refine_benchmark", name="test hipip"
        )
        self.add_test_command(
            "phenix_regression.wizards.test_all_parallel",
            args=["nproc=8"],
            name="test wizards",
        )
...
class PhenixExternalRegression(PhenixBuilder):
    EXTERNAL_CODEBASES = ["afitt", "rosetta"]
    ...
    def get_environment(self, add_build_python_to_path=True):
        ...
        if add_build_python_to_path:
            ...
            environment["PATH"] = "%s:%s" % (
                os.path.join("build", "bin"),  # os.getcwd(),
                old_path,
            )
        return environment

    def write_environment(self, env, filename="setpaths_externals"):
        ...
        self.add_step(
            self.shell(
                command=[
                    "python",
                    "-c",
                    'import os; open("%s","w").write("""%s""" %% os.environ)'
                    % (fname, outl),
                ],
                workdir=[_BUILD_DIR],
    ...
    def add_make(self):
        ...
        for name, command, workdir in [
            ["AFITT - untar", ["tar", "xvf", "%s.gz" % afitt_version], ["modules"]],
            [
                "Rosetta - untar",
            ...
            if self.subcategory:
                if name.lower().find(self.subcategory) == -1:
                    continue
            haltOnFailure = True
            self.add_step(
                self.shell(
    ...
    def add_tests(self):
        ...
        if self.subcategory in [None, "amber"]:
            self.add_test_command(
                "amber.run_tests", env=self.get_environment(), haltOnFailure=False
            )
        # rosetta refine
        if self.subcategory in [None, "rosetta"]:
            self.add_test_command("rosetta.run_tests", env=self.get_environment())
        # MR rosetta
        if self.subcategory in [None, "rosetta"]:
            self.add_test_command(
        ...
        # afitt
        if self.subcategory in [None, "afitt"]:
            self.add_test_command("afitt.run_tests", env=self.get_environment())
        # erraser
        if self.subcategory in [None, "rosetta"]:
            self.add_test_command("erraser.run_tests", env=self.get_environment())
        # GLR
        if self.subcategory in [None, "glr"]:
            self.add_test_command("elbow.run_glr_tests", haltOnFailure=False)
...
class QRBuilder(PhenixBuilder):
    ...
    def add_make(self):
        ...
        for pi in pip_installs:
            instructions.append(
                [
                    "Q|R pip %s" % pi,
                    [self.python_base, "-m", "pip", "install", pi],
                    ["modules"],
                ]
            )
    ...
    def get_hot(self):
        if self.user == "builder":
            return PhenixBuilder.get_hot(self)
    ...
    def add_tests(self):
        self.add_test_command(
            "qr.build_interfaces", haltOnFailure=False, env=self.get_environment()
        )
        self.add_test_command("qr.test", haltOnFailure=True, env=self.get_environment())
