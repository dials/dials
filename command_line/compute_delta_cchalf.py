...
class Script(object):
  ...
  def __init__(self, params, experiments, reflections):
    ...
    self.params = params
    self.delta_cchalf_i = {}
    self.results_summary = {'dataset_removal' : {
  ...
  def run(self):
    ...
    statistics = PerImageCChalfStatistics(data.miller_index, data.identifiers,
      data.dataset, data.images, data.intensity, data.variance, data.unit_cell,
      data.space_group, self.params.nbins, self.params.dmin, self.params.dmax,
      self.params.mode, self.params.group_size)
    ...
    if self.experiments and len(self.reflections) == 1:
      cutoff_value = self._calculate_cutoff_value(self.delta_cchalf_i,
        self.params.stdcutoff)
      ...
      if self.params.mode == "dataset":
        filtered_reflections = self.remove_datasets_below_cutoff(
          self.experiments, self.reflections[0], ids_to_remove, self.results_summary)
      elif self.params.mode == "image_group":
        filtered_reflections = self.remove_image_ranges_below_cutoff(
          self.experiments, self.reflections[0], ids_to_remove,
          statistics.image_group_to_expid_and_range, self.results_summary)
  ...
  def read_experiments(self, experiments, reflections):
    ...
    # Get the MTZ file
    return self.DataRecord(unit_cell=unit_cell, space_group=space_group,
      miller_index=miller_index, dataset=index, intensity=intensity,
      variance=variance, identifiers=identifiers, images=images)
  ...
  def read_mtzfile(self, filename):
    ...
    # Get the MTZ file
    return self.DataRecord(unit_cell=unit_cell_list, space_group=space_group,
      miller_index=miller_index, dataset=dataset, intensity=intensity,
      variance=variance)
  ...
  def sort_deltacchalf_values(delta_cchalf_i, results_summary):
    ...
    sorted_index = sorted(range(len(datasets)),
      key=lambda x: delta_cchalf_i[datasets[x]])
  ...
  @staticmethod
  def remove_image_ranges_below_cutoff(experiments, reflections,
    ids_to_remove, image_group_to_expid_and_range, results_summary):
    ...
    results_summary['dataset_removal'].update({
      ...
      'n_reflections_removed' : n_valid_reflections - n_valid_filtered_reflections})
    return output_reflections

  @staticmethod
  def remove_datasets_below_cutoff(experiments, reflections, ids_to_remove,
    results_summary):
  ...
  def write_experiments_and_reflections(self):
    ...
    if self.experiments and len(self.reflections) == 1:
      logger.info('Saving %d reflections to %s', len(self.reflections[0]),
...
def run(args=None, phil=phil_scope):
  ...
  parser = OptionParser(usage=usage, phil=phil, epilog=__doc__,
