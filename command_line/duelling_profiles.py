...
Examples::
  ...
  dev.dials.duelling_profiles models.expt integrated.refl
...
def model_reflection_rt0(reflection, experiment, params):
    ...
    for i in range(int(round(i0 * scale))):
        ...
        if params.rs_node_size > 0:
            ...
            dp0 = matrix.col(
                (random.gauss(0, ns), random.gauss(0, ns), random.gauss(0, ns))
            )
            p0 += dp0
...
def main(reflections, experiment, params):
    ...
    for j, reflection in enumerate(reflections):
        ...
        if ids:
            if j in ids:
                result = globals()["model_reflection_%s" % method](
                    reflection, experiment, params
                )
