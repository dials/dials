...
def slice_datablocks(datablocks, image_ranges):
    ...
    if len(scans) != len(image_ranges):
        raise Sorry(
            "Unique scans in the input datablock list and supplied image_ranges are not of the same length"
        )
...
class Script(object):
    ...
    def run(self):
        ...
        if params.block_size is not None:
            ...
            if slice_dbs:
                sliced = [
                    slice_datablocks(datablocks, [sr])[0] for sr in params.image_range
                ]
