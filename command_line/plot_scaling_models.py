...
def plot_scaling_models(argv):
    ...
    if len(experiments) != 1:
        print(
            (
                "Checking for the existence of a reflection table containing {sep}"
                "multiple scaled datasets. {sep}"
            ).format(sep="\n")
        )
    ...
    if len(experiments) == 1:
        ...
        if isinstance(experiment.scaling_model, Model.PhysicalScalingModel):
            if (
                "scale" in experiment.scaling_model.configdict["corrections"]
                or "decay" in experiment.scaling_model.configdict["corrections"]
            ):
                plot_smooth_scales(
                    params,
                    experiment,
            ...
            if "absorption" in experiment.scaling_model.configdict["corrections"]:
                plot_absorption_surface(
                    experiment, outputfile=str(params.output.absorption_out) + ".png"
                )
...
def plot_multi(params, experiment, reflection, j):
    ...
    if isinstance(experiment.scaling_model, Model.PhysicalScalingModel):
        ...
        if "absorption" in experiment.scaling_model.configdict["corrections"]:
            plot_absorption_surface(
                experiment,
                outputfile=str(params.output.absorption_out)
                + "_"
                + str(j + 1)
                + ".png",
            )
...
def plot_smooth_scales(params, experiment, outputfile=None):
    ...
    if "scale" in experiment.scaling_model.configdict["corrections"]:
        smoother_phis, parameters, parameter_esds, sample_values, sample_scales = ScalingModelAnalyser.get_smooth_plotting_data_from_model(
    ...
    if "decay" in experiment.scaling_model.configdict["corrections"]:
        ...
        ax2.plot(
            sample_values,
            np.log(sample_scales) * 2.0,
            label="smoothly varying \nB-factor",
        )  # convert scales to B values
        if params.output.with_errors:
            if params.output.limit_range_to_obs:
                ax2.errorbar(
                    smoother_phis[1:-1],
...
def plot_absorption_surface(experiment, outputfile=None):
    ...
    ax.set_yticks(
        [0, (STEPS - 1) / 4.0, (STEPS - 1) / 2.0, 3.0 * (STEPS - 1) / 4.0, STEPS - 1]
    )
...
def plot_2D_decay_correction(experiment, reflections, outputfile=None):
    ...
    reflections = reflections.select(
        ~reflections.get_flags(reflections.flags.user_excluded_in_scaling)
    )
    reflections = reflections.select(
        reflections.get_flags(reflections.flags.integrated)
    )
    configdict = experiment.scaling_model.configdict
    reflections["normalised_res_values"] = (
        (1.0 / (reflections["d"] ** 2)) - configdict["resmin"]
    ) / configdict["res_bin_width"]
    reflections["norm_time_values"] = (
        reflections["xyzobs.px.value"].parts()[2] * configdict["time_norm_fac"]
    )
    reflections["norm_time_values"] = reflections["norm_time_values"] - min(
        reflections["norm_time_values"]
    )
    ...
    rel_values = flex.double(
        np.linspace(
            0,
            int_rel_max - int_rel_min,
            ((int_rel_max - int_rel_min) / 0.1) + 1,
            endpoint=True,
        )
    )
    ...
    xlabels = np.arange(
        0, (time_rot_int * (int_rel_max - int_rel_min)) + 0.001, time_rot_int * 3
    )
...
def plot_2D_modulation_correction(experiment, reflections, outputfile=None):
    ...
    configdict = experiment.scaling_model.configdict
    nxdet = (
        reflections["xyzobs.px.value"].parts()[0] - configdict["xmin"]
    ) / configdict["x_det_bin_width"]
    nydet = (
        reflections["xyzobs.px.value"].parts()[1] - configdict["ymin"]
...
def plot_3D_absorption_correction(experiment, reflections, outputfile=None):
    ...
    parameters_2D = np.reshape(
        list(absorption_factor.parameters), (n_time_bins, n_abs_bins)
    ).T
