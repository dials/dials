    ...
    def __init__(self):
        ...
        if params.input.datablock:
            if params.input.experiments:
                raise Sorry(
                    "Please provide either datablocks or experiment lists, "
                    "not a combination of the two"
                )
            if len(params.input.datablock) != 2:
                raise Sorry(
                    "Please provide two datablocks or experiment lists as input"
                )

            warnmsg = (
                "WARNING: The {0} datablock contains more than one "
