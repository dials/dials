...
def run(args):
    ...
    for j, experiment in enumerate(experiments):
        ...
        abc_names = "abc"
        distances = [
            (r2d * (min(axis.angle(_a), pi - axis.angle(_a))), k)
        ...
        print("Closest (original) axis: %s* %.2f" % (abc_names[close[1]], close[0]))
        ...
        results.sort()
        print(
            "Best reindex op for experiment %d: %12s (%.3f)"
        ...
        if results[0][0] > (5 * pi / 180.0):
            ...
            print(
                "          angle: %.4f degrees" % (180.0 * results[0][2].angle() / pi)
            )
