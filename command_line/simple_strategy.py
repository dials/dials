...
def run(args):
    ...
    usage = "%s [options] indexed.expt" % (libtbx.env.dispatcher_name)
...
class Strategy(object):
    def __init__(
        ...
        Btot = 1 - 3 * (4 * theta_max_rad - math.sin(4 * theta_max_rad)) / (
            32 * (math.sin(theta_max_rad) ** 3)
        )
        print(Btot)
        ...
        scan.set_image_range(
            (image_range[0], image_range[0] + int(max_rotation / oscillation[1]))
        )
        self.predict()
    ...
    def predict(self):
        ...
        self.predicted = flex.reflection_table.from_predictions(
            self.experiment, force_static=True, dmin=self.d_min
        )
    ...
    def determine_cutoffs(self, min_frac_new=None):
        if min_frac_new is None:
            self.min_frac_new = min_frac_new
        self.cutoff_non_anom = (
            self.degrees_per_bin
            * self.determine_fraction_new_cutoff(self.frac_new_ref, min_frac_new)
        )
    ...
    def __str__(self):
        ...
        output.append(
            "Suggested cutoff (non-anom): %.2f degrees" % self.cutoff_non_anom
        )
        output.append(
            "  (completeness: %.2f %%)"
            % (
                100
                * self.ieither_completeness[
                    int(self.cutoff_non_anom / self.degrees_per_bin)
                ]
            )
        )
        ...
        output.append(
            ...
            % (
                100
                * self.iboth_completeness[int(self.cutoff_anom / self.degrees_per_bin)]
            )
        )
        return "\n".join(output)
