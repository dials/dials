...
def main(argv):
    ...
    if len(experiments) != 1:
        print(
            (
                "Checking for the existence of a reflection table {sep}"
                "containing multiple scaled datasets {sep}"
            ).format(sep="\n")
        )
    ...
    if isinstance(scaler, MultiScalerBase):
        ...
        for i, single_scaler in enumerate(scaler.single_scalers):
            good_sel = ~(
                single_scaler.reflection_table.get_flags(
                    single_scaler.reflection_table.flags.bad_for_scaling, all=False
                )
            )
            Ih_table = IhTable(
                [single_scaler.reflection_table.select(good_sel)], scaler.space_group
            ).blocked_data_list[0]
            error_model = BasicErrorModel(Ih_table)
            if (
                not "error_model_parameters"
                in single_scaler.experiments.scaling_model.configdict
            ):
                raise Sorry(
                    """No error model found in experiments file - likely cause is that
          no error model was optimised for this dataset."""
            ...
            if params.output.combine_plots:
                normal_probability_plot(
                    ...
                    filename="none",
                    gs=gs,
                    opacity=1.0 / len(reflections),
                    label=label,
                    color=colors[i],
                )
            else:
                normal_probability_plot(
                    error_model, filename="error_models_" + str(i) + ".png", label=label
                )
        if params.output.combine_plots:
            plt.tight_layout()
        ...
        good_sel = ~(
            scaler.reflection_table.get_flags(
                scaler.reflection_table.flags.bad_for_scaling, all=False
            )
        )
        Ih_table = IhTable(
            [scaler.reflection_table.select(good_sel)], scaler.space_group
        ).blocked_data_list[0]
        error_model = BasicErrorModel(Ih_table)
        ...
        if params.output.plot_labels:
            ...
            label = None
        normal_probability_plot(
            error_model, filename=params.output.plot_out, label=label
        )


def normal_probability_plot(
    error_model, filename, mode="full", gs="none", opacity=1.0, label=None, color="b"
):
    ...
    rankits = flex.double(
        [norm.quantile((i + 1 - a) / (n + 1 - (2 * a))) for i in xrange(n)]
    )
    if mode == "full":
        plt.figure(figsize=(8, 8))
        gs = gridspec.GridSpec(2, 2)
