...
conda_base = precommit_home.join("..").join("..").join("conda_base")
if not conda_base.check():
    conda_base = None
...
def precommitbx_template(python):
    if conda_base:
        file_content = (
            ...
            "fi",
            "source "
            + conda_base.join("etc").join("profile.d").join("conda.sh").strpath,
            "conda activate " + conda_base.strpath,
...
def conda_path():
    if not conda_base:
        return False
    if os.name == "nt":
        conda_exe = conda_base.join("Scripts").join("conda.exe")
    else:
        conda_exe = conda_base.join("bin").join("conda")
    if not conda_exe.check():
        return False
    return conda_exe


def conda_info(conda):
    result = clean_run([conda, "info", "--json"])
    if result.returncode:
        return False
...
def main():
    ...
    if sys.version_info[0] == 3:
        ...
        print(
            "Base Python:", (GREEN if sys.version_info.minor >= 6 else NC) + py_ver + NC
        )
        try:
        ...
        if python:
            ...
            if pc_ver == precommitbx_version:
                ...
                pc_ver = (
                    YELLOW + pc_ver + NC + " (expected: " + precommitbx_version + ")"
                )
                changes_required = True
            else:
